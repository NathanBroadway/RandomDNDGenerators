...
namespace Classes
{
    class Program
    {
        ...
        {
            Console.Write("What would you like to do?\n[1] Access Spell Effects\n[2] Access Sorcerer Effects\n[3] Access Artificer Effects\n[4] Access Barbarian Effects\n[4] Access Druid Effects\n[6] Access Ranger Effects\n[7] Access Warlock Effects\n[5] Access Monk Effects\nInput: ");
        ...
        private static void AccessWarlock()
        {
            ...
            {
                Console.Clear();
                string input = AskForInput("Warlock", new List<string>{ "Genie Kind Table", "Genie's Vessel Table" });
                Console.Clear();
                ...
                switch (response.ToLower())
                {
                    ...
                    case "genie":
                    case "kind":
                        listOfStrings = new List<string> { "Dao - Earth", "Djinni - Air", "Efreeti - Fire", "Marid - Water" };
                        Console.WriteLine(GetFromList(listOfStrings));
                        break;
                    case "2":
                    case "genie's":
                    case "vessel":
                        listOfStrings = new List<string> { "Oil Lamp", "Urn", "Ring with a compartment", "Stoppered bottle", "Hollow statuette", "Ornate lantern" };
                        Console.WriteLine(GetFromList(listOfStrings));
                        break;
                }
                Console.WriteLine("Look at Monk Detail? [Y/N]");
            } while (Console.ReadKey().Key != ConsoleKey.N);
        }

        private static string AskForInput(string title, List<string> listOfOptions)
        {
        ...
        private static void AccessRanger()
        {
            ...
            {
                Console.Clear();
                Console.Write("What would you like to do?\n[1] Access Drakewarden Origin Table\n[2] Access Feywild Gifts Table\n[3] Access Swarm Appearance Table\nInput: ");
                var response = Console.ReadLine();
                Console.Clear();
                List<string> listOfStrings;
                switch (response.ToLower())
        ...
        private static void AccessMonk()
        {
            ...
            {
                Console.Clear();
                Console.Write("What would you like to do?\n[1] Access Merciful Mask Table\n[2] Access Ascendant Dragon Origin Table\nInput: ");
                var response = Console.ReadLine();
                Console.Clear();
                List<string> listOfStrings;
                switch (response.ToLower())
                {
                    ...
                    case "merciful":
                    case "mask":
                        listOfStrings = new List<string> { "Raven", "Blank and white", "Crying visage", "Laughing visage", "Skull", "Butterfly" };
                        Console.WriteLine(GetFromList(listOfStrings));
                        break;
                    case "2":
                    case "ascendant":
        ...
        private static void AccessDruid()
        {
            do
            {
                ...
                switch (response.ToLower())
                {
                    ...
                    case "star":
                    case "map": 
                        listOfStrings = new List<string> { "A scroll covered with depictions of constellations", "A stone tablet with fine holes drilled through it", "A speckled owlbear hide, tooled with raised marks", "A collection of maps bound in an ebony cover", "A crystal that projects starry patterns when placed before a light", "Glass disks that depict constellations" };
                        Console.WriteLine(GetFromList(listOfStrings));
                        break;
                }
                Console.WriteLine("Look at Druid Detail? [Y/N]");
            } while (Console.ReadKey().Key != ConsoleKey.N);
        }
        ...
        private static void AccessBarbarian()
        {
            ...
            {
                Console.Clear();
                Console.Write("What would you like to do?\n[1] Access Origin of the Beast Table\n[2] Access Wild Magic Table\nInput: ");
                var response = Console.ReadLine();
                Console.Clear();
                List<string> listOfStrings;
                switch (response.ToLower())
        ...
        private static void AccessArtificer()
        {
            ...
            {
                Console.Clear();
                Console.Write("What would you like to do?\n[1] Access Experimental Elixir Table\nInput: ");
                var response = Console.ReadLine();
                Console.Clear();
                List<string> listOfStrings;
                switch (response.ToLower())
        ...
        private static void AccessSorcerer()
        {
            ...
            {
                Console.Clear();
                Console.Write("What would you like to do?\n[1] Access Aberrant Origins Table\n[2] Access Manifestations of Order Table\n[3] Access Shadow Sorcerer Quirks Table\n[4] Access Wild Magic Surge Table\nInput: ");
                var response = Console.ReadLine();
                Console.Clear();
                List<string> listOfStrings;
                switch (response.ToLower())
        ...
        private static void AccessSpells()
        {
            ...
            {
                Console.Clear();
                Console.Write("What would you like to do?\n[1] Access Chaos Bolt\n[2] Access Chromatic Orb\n[3] Access Find Familiar\n[4] Access Dragon's Breath\n[5] Access Elemental Bane\n[6] Access Reincarnation\n[7] Access Conjure Minor Elementals\n[8] Access Conjure Elemental\n[9] Access Conjure Fey\n[10] Access Conjure Celestial\n[11] Access Conjure Greater Demon\n[12]\nAccess Animal Shapes\n[13] Access Conjure Woodland Beings\n[14] Access Conjure Animals\n[15] Access Conjure Lesser Demons\nInput: ");
                var response = Console.ReadLine();
                Console.Clear();
                List<string> listOfStrings;
                string damageType;
                Dictionary<string, string> dictOfMonters;
                string monsterName;
                string monsterCR;
                switch (response.ToLower())
                {
                    ...
                    case "chromatic":
                    case "orb":
                        listOfStrings = new List<string> { "Acid", "Cold", "Fire", "Lightning", "Poison", "Thunder" };
                        Console.WriteLine($"You hurl a 4-inch-diameter sphere of energy at a creature that you can see within range. Make a ranged spell attack against the target. If the attack hits, the creature takes {DiceRoller.RollDice("3d8")} {GetFromList(listOfStrings)} damage.");
                        break;
                    case "3":
                    case "find familiar":
                    case "find":
                    case "familiar":
                        listOfStrings = new List<string> { "Bat", "Cat", "Crab", "Frog (Toad)", "Hawk", "Lizard", "Octopus", "Owl", "Poisonous Snake", "Quipper", "Rat", "Raven", "Sea Horse", "Spider", "Weasel" };
                        var listOfStrings2 = new List<string> { "Celestial", "Fey", "Fiend" };
                        Console.WriteLine($"You gain the service of a familiar, a spirit that takes the form of an {GetFromList(listOfStrings2)} {GetFromList(listOfStrings)}. Appearing in an unoccupied space within range, the familiar has the statistics of the chosen form.");
                        break;
                    case "4":
                    ...
                    case "dragon":
                    case "breath":
                        listOfStrings = new List<string> { "Acid", "Cold", "Fire", "Lightning", "Poison", "Acid", "Cold", "Fire", "Lightning", "Thunder" };
                        damageType = GetFromList(listOfStrings);
                    ...
                    case "elemental":
                    case "bane":
                        listOfStrings = new List<string> { "Acid", "Cold", "Fire", "Lightning", "Thunder" };
                        damageType = GetFromList(listOfStrings);
                    ...
                    case "6":
                    case "reincarnation":
                        listOfStrings = new List<string> { "Dragonborn", "Hill Dwarf", "Mountain Dwarf", "Dark Elf", "High Elf", "Wood Elf", "Forest Gnome", "Rock Gnome", "Half-Elf", "Half-Orc", "Lightfoot Halfling", "Stout Halfling", "Human", "Tiefling" };
                        Console.WriteLine($"You touch a dead Humanoid or a piece of a dead Humanoid. Provided that the creature has been dead no longer than 10 days, the spell forms a new adult {GetFromList(listOfStrings)} body for it and then calls the soul to enter that body. If the target's soul isn't free or willing to do so, the spell fails.\n\tThe reincarnated creature recalls its former life and experiences. It retains the capabilities it had in its original form, except it exchanges its original race for the new one and changes its racial Traits accordingly.");
                        break;
                    case "7":
                    case "conjure minor elementals":
                        dictOfMonters = ListOfMonsters(MonsterTypes.elemental, "2");
                        monsterName = GetFromList(dictOfMonters.Keys.ToList());
                        monsterCR = dictOfMonters[monsterName];
                        switch (monsterCR)
                        {
                            case "2":
                                monsterCR = "1";
                        ...
                        Console.WriteLine($"You summon {monsterCR} {monsterName}{(monsterCR != "1" ? "s" : "")} that appear in unoccupied spaces that you can see within range.\n\tAn elemental summoned by this spell disappears when it drops to 0 hit points or when the spell ends.\n\tThe summoned creatures are friendly to you and your companions. Roll initiative for the summoned creatures as a group, which has its own turns. They obey any verbal commands that you issue to them (no action required by you). If you don't issue any commands to them, they defend themselves from hostile creatures, but otherwise take no actions.");
                        break;
                    case "8":
                    case "conjure elemental":
                        dictOfMonters = ListOfMonsters(MonsterTypes.elemental, "5");
                        Console.WriteLine($"You summon {GetFromList(dictOfMonters.Keys.ToList())} that appear in unoccupied spaces that you can see within range.\n\tAn elemental summoned by this spell disappears when it drops to 0 hit points or when the spell ends.\n\tThe summoned creatures are friendly to you and your companions. Roll initiative for the summoned creatures as a group, which has its own turns. They obey any verbal commands that you issue to them (no action required by you). If you don't issue any commands to them, they defend themselves from hostile creatures, but otherwise take no actions.");
                        break;
                    case "9":
                    case "conjure fey":
                    ...
                    case "conjure celestial":
                        listOfStrings = ListOfMonsters(MonsterTypes.celestial, "4").Keys.ToList();
                        Console.WriteLine($"You summon a {GetFromList(listOfStrings)}, which appears in an unoccupied space that you can see within range. The celestial disappears when it drops to 0 hit points or when the spell ends.\n\tThe celestial is friendly to you and your companions for the duration. Roll initiative for the celestial, which has its own turns. It obeys any verbal commands that you issue to it (no action required by you), as long as they don't violate its alignment. If you don't issue any commands to the celestial, it defends itself from hostile creatures but otherwise takes no actions.");
                        break;
                    case "11":
                    case "conjure greater demon":
                        listOfStrings = ListOfMonsters(MonsterTypes.demon, "4").Keys.ToList();
                        Console.WriteLine($"You utter foul words, summoning a {GetFromList(listOfStrings)} from the chaos of the Abyss. The demon appears in an unoccupied space you can see within range, and the demon disappears when it drops to 0 hit points or when the spell ends.\n\tRoll initiative for the demon, which has its own turns. When you summon it and on each of your turns thereafter, you can issue a verbal command to it (requiring no action on your part), telling it what it must do on its next turn. If you issue no command, it spends its turn attacking any creature within reach that has attacked it.\n\tAt the end of each of the demon’s turns, it makes a Charisma saving throw. The demon has disadvantage on this saving throw if you say its true name. On a failed save, the demon continues to obey you. On a successful save, your control of the demon ends for the rest of the duration, and the demon spends its turns pursuing and attacking the nearest non-demons to the best of its ability. If you stop concentrating on the spell before it reaches its full duration, an uncontrolled demon doesn’t disappear for 1d6 rounds if it still has hit points.\n\tAs part of casting the spell, you can form a circle on the ground with the blood used as a material component. The circle is large enough to encompass your space. While the spell lasts, the summoned demon can’t cross the circle or harm it, and it can’t target anyone within it. Using the material component in this manner consumes it when the spell ends.\n\tAt Higher Levels. When you cast this spell using a spell slot of 5th level or higher, the challenge rating increases by 1 for each slot level above 4th.");
                        break;
                    case "12":
                    case "animal shapes":
                        listOfStrings = ListOfMonsters(MonsterTypes.beast, "4").Keys.ToList();
                        Console.WriteLine($"Your magic turns others into Beasts. Choose any number of willing Creatures that you can see within range. You transform each target into the form of a {GetFromList(listOfStrings)}. On subsequent turns, you can use your Actions to transform affected Creatures into new forms.\n\tThe transformation lasts for the Duration for each target, or until the target drops to 0 Hit Points or dies. You can choose a different form for each target. A target's game Statistics are replaced by the Statistics of the chosen beast, though the target retains its Alignment and Intelligence, Wisdom, and Charisma scores. The target assumes the Hit Points of its new form, and when it reverts to its normal form, it returns to the number of hit point it had before it transformed. If it reverts as a result of dropping to 0 Hit Points, any excess damage carries over to its normal form. As long as the excess damage doesn't reduce the creature's normal form to 0 Hit Points, it isn't knocked Unconscious. The creature is limited in the Actions it can perform by the Nature of its new form, and it can't speak or cast Spells.\n\tThe target's gear melds into the new form. The target can't activate, wield, or otherwise benefit from any of its Equipment.");
                        break;
                    case "13":
                    case "conjure woodland beings":
                        dictOfMonters = ListOfMonsters(MonsterTypes.fey, "2");
                        monsterName = GetFromList(dictOfMonters.Keys.ToList());
                        monsterCR = dictOfMonters[monsterName];
                        switch (monsterCR)
                        {
                    ...
                    case "conjure animals":
                        dictOfMonters = ListOfMonsters(MonsterTypes.beast, "2");
                        monsterName = GetFromList(dictOfMonters.Keys.ToList());
                        monsterCR = dictOfMonters[monsterName];
                        switch (monsterCR)
                        {
                    ...
                    case "conjure lesser demons":
                        dictOfMonters = ListOfMonsters(MonsterTypes.demon, "2");
                        monsterName = GetFromList(dictOfMonters.Keys.ToList());
                        monsterCR = dictOfMonters[monsterName];
                        switch (monsterCR)
                        {
